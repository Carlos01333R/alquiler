...
export default function CotizacionesPage() {
  ...
  const handleEliminar = async () => {
    ...
    try {
      ...
      if (error) throw error
      toast.success(`${ids.length} cotización${ids.length > 1 ? 'es' : ''} eliminada${ids.length > 1 ? 's' : ''}`)
      ...
      <div className="bg-white rounded-lg shadow-md">
        ...
        <div className="p-6 border-b">
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">Documentos tipo COT</p>
            </div>
            <button
              onClick={() => router.push('/dashboard/cotizaciones/nuevo')}
              className="px-4 py-2 bg-yellow-500  rounded-md hover:bg-yellow-600 flex items-center gap-2 text-sm font-medium transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="#fff" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
              <p className="text-white">Nueva Cotización</p>
              ...
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="px-4 py-3 w-10">
                    <input type="checkbox" checked={todosSeleccionados} onChange={toggleTodos} className="w-4 h-4 accent-yellow-500 cursor-pointer" />
                  </th>
                  ...
                  return (
                    <tr key={doc.id} className={`hover:bg-gray-50 transition-colors ${sel ? 'bg-red-50' : ''}`}>
                      <td className="px-4 py-4">
                        <input type="checkbox" checked={sel} onChange={() => toggleSeleccion(doc.id)} className="w-4 h-4 accent-yellow-500 cursor-pointer" />
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-3">
                          <button onClick={() => router.push(`/dashboard/cotizaciones/${doc.id}/totales`)} className=" hover:text-yellow-800 text-sm font-medium cursor-pointer">
                             <Eye className="w-5 h-5 text-yellow-600" />
                          </button>
                          <button
                            onClick={() => router.push(`/dashboard/cotizaciones/${doc.id}/detalles?empresa_id=${doc.empresa_id}&modo=editar`)}
        ...
        {!loading && filtradas.length > 0 && (
          <div className="px-6 py-3 border-t bg-gray-50 text-xs text-gray-400 flex justify-between">
            <span>{filtradas.length} cotización{filtradas.length !== 1 ? 'es' : ''}</span>
