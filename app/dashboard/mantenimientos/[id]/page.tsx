...
export default function MantenimientoDetailPage() {
  ...
  if (!item)
    return <p className="text-muted-foreground">Mantenimiento no encontrado</p>

  const actividades: { nombre: string; completada?: boolean | undefined }[] =
    ...
    <div className="space-y-6 w-[90%] md:max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        <InfoCard
          icon={Tag}
          label="Tipo"
          value={<StatusBadge value={item.tipo} />}
        />
        <InfoCard
          icon={AlertCircle}
          label="Prioridad"
          value={<StatusBadge value={item.prioridad} />}
        />
        <InfoCard
          icon={User}
          label="Cliente"
        ...
        <InfoCard
          icon={Calendar}
          label="Creado"
          value={new Date(item.created_at).toLocaleDateString("es-CO", { dateStyle: "long" })}
        />
      </div>
