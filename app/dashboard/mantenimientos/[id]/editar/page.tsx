...
export default function MantenimientoEditPage() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    setUploading(true)
    try {
      const fileName = `mantenimientos/${id}/${Date.now()}_${file.name.replace(/\s+/g, "_")}`

      // Si ya hay un archivo, eliminarlo
  ...
  const handleRemoveFile = async () => {
    ...
    try {
      ...
      await supabase.from("mantenimientos").update({ archivo_adjunto: null }).eq("id", id)
      ...
      <div className="bg-white rounded-xl border shadow-sm p-6 space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Sube documentos PDF relacionados a este mantenimiento (máx. 10 MB)
          </p>
        </div>
